...
public class JdbcOrderRepository implements OrderRepository {
    ...
    private static final String FIND_ORDERS_SQL =
            "SELECT o.id, c.email, c.address, o.order_date, o.status, b.title, o.price " +
                    "FROM ORDERS o " +
    ...
    public Long save(Order order) {
        ...
        try {
            template.update(SAVE_ORDER_SQL, getOrderParameterSource(order.getOrderDate(), order.getDeliveryStatus(), order.getCustomerId(), order.getPrice()), keyHolder);
        } catch (Exception e) {
    ...
    private boolean isSameOrderId(ResultSet resultSet, LinkedList<OrderDTO.OrderHistoryQueryRes> orderHistoryQueryResList) throws SQLException {
        return orderHistoryQueryResList.size() != 0
                && orderHistoryQueryResList.peekLast().orderId() == resultSet.getLong("id");
    }
